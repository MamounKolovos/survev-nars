...
export class Player extends BaseGameObject {
    ...
    get cullingZoom(): number {
        return this._cullingZoom;
    }
    ...
    get inventory(): Record<InventoryItem, number> {
        return this.invManager.items as Record<InventoryItem, number>;
    }
    ...
    promoteToRole(role: string) {
        ...
        if (this.role === "medic" && !this.game.playerBarn.medics.includes(this)) {
        ...
        if (roleDef.defaultItems) {
            ...
            for (const [key, value] of Object.entries(roleDef.defaultItems.inventory)) {
                const invKey = key as InventoryItem;
                ...
                const invDef = GameObjectDefs[invKey];
                if (
                    invDef.type == "scope" &&
                ...
                const spaceLeft =
                    this.bagSizes[invKey][backpackLevel] - this.inventory[invKey];
                this.inventory[invKey] += math.clamp(value, 0, spaceLeft);
            }
    ...
    handleInput(msg: net.InputMsg): void {
        ...
        if (!v2.eq(this.dirNew, msg.toMouseDir)) {
            this.setPartDirty();
            this.dirOld = v2.copy(this.dirNew);
