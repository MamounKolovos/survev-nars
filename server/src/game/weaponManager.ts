...
export class WeaponManager {
    ...
    getTrueAmmoStats(weaponDef: GunDef) {
        return this.getAmmoStats(weaponDef);
    }

    scheduleReload(_delay?: number) {
        this.scheduledReload = true;
    }
    ...
    tryReload() {
        ...
        const invAmmo = this.player.invManager.isValid(weaponDef.ammo)
            ? this.player.invManager.get(weaponDef.ammo as InventoryItem)
            : 0;
        const conditions = [
            ...
            this.weapons[this.curWeapIdx].ammo >= stats.maxClip,
            !isInfinite && invAmmo <= 0,
        ];
